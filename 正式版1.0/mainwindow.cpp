...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
    ...
    resize(1050,920);
    setWindowTitle(tr("Fight For Your Girl!-The combination of Minecraft and PVZ"));
...
void MainWindow::CreateActions() {
    ...
    newGame = new QAction(tr("新的游戏"), this);
    saveGame = new QAction(tr("存档"), this);
    loadGame = new QAction(tr("重载游戏存档"), this);
    exitGame = new QAction(tr("退出游戏"), this);
    cheatHp = new QAction(tr("血量+100"), this);
    cheatAttack = new QAction(tr("攻击+10"), this);
    cheatDefend = new QAction(tr("防御+10"), this);
    cheatMoney = new QAction(tr("金币+50"), this);
    cheatLevel = new QAction(tr("等级+1"), this);
    cheatYellowKey = new QAction(tr("黄色钥匙+1"), this);
    cheatBlueKey = new QAction(tr("蓝色钥匙+1"), this);
    cheatRedKey = new QAction(tr("红色钥匙+1"), this);
    cheatBook = new QAction(tr("怪物图谱"), this);
    cheatSword = new QAction(tr("剑"), this);
    cheatShield = new QAction(tr("盾牌"), this);
    cheatMode = new QAction(tr("无敌模式"), this);

    newGame->setShortcut(tr("N"));
    saveGame->setShortcut(tr("S"));
    loadGame->setShortcut(tr("L"));
    exitGame->setShortcut(tr("Q"));//上述代码用于创造快捷键
    ...
    connect(cheatMoney, SIGNAL(triggered()), cheats, SLOT(slotCheatCoin()));
...
void MainWindow::CreateMenus() {
    ...
    QMenu *fileMenu = menuBar()->addMenu(tr(" 目录 "));
...
//创建一个名为 "Cheat!" 的菜单，并将其添加到主窗口的菜单栏上。
    QMenu *cheatMenu = menuBar()->addMenu(tr(" 我玩不下去了 "));
...
void MainWindow::slotBook() {               // 绘制book
    ...
    for (int i = 11; i < 24; i++)           //写出怪物的信息
        if (exist[i]) {
            ...
            AddTextItem(-35, -28 + tmp*40, "血量:", 8, kGreen);
            AddTextItem(63, -28 + tmp*40, "攻击:", 8, kGreen);
            AddTextItem(183, -28 + tmp*40, "防御:", 8, kGreen);
            AddTextItem(196, -12 + tmp*40, "躲避:", 8, kPurple);
            AddTextItem(343, -12 + tmp*40, "暴击:", 8, kPurple);
            AddTextItem(-40, -12 + tmp*40, "经验:", 8, kBlue);
            AddTextItem(62, -12 + tmp*40, "金币:", 8, kBlue);
            AddTextItem(280, -28 + tmp*40, "预期伤害:", 8,  kYellow);
...
void MainWindow::slotEvent(QString str) {
    ...
    else if (str == "boy") player.ChooseKind(1);
    else if (str == "girl") player.ChooseKind(0);
...
void MainWindow::slotIdeas(){
    Clear();
    AddPictureItem(-102,0,"Ideas");
    view->SetStatus("Ideas");
}
...
void MainWindow::slotFight(int num) {       // 模拟战斗并生成具体的战斗结果
    ...
    AddTextItem(75, 171, "暴击 : " + QString::number(playerCrit, kDecimal), 12, kRed);
    AddTextItem(215, 171, "暴击 : " + QString::number(monsterCrit, kDecimal), 12, kRed);
    AddTextItem(75, 188, "躲避 : " + QString::number(playerMiss, kDecimal), 12, kGreen);
    AddTextItem(215, 188, "躲避 : " + QString::number(monsterMiss, kDecimal), 12, kGreen);
    AddTextItem(75, 265, "金钱 + " + QString::number(monsters[num].GetMoney(), kDecimal), 12, kBlue);
    AddTextItem(210, 265, "经验 + " + QString::number(monsters[num].GetExp(), kDecimal), 12, kBlue);
